...
export default class SquadLayerFilter extends Layers {
  ...
  static buildFromDidYouMeanList(layerNames, activeLayerFilter) {
    ...
    for (const layerName of layerNames) {
      const layer = Layers.getLayerByDidYouMean(layerName, Layers.getLayerNames());
  ...
  static buildFromFile(path, activeLayerFilter, delimiter = '\n') {
    ...
    const validLayerNames = Layers.getLayerNames();

    for (const line of lines) {
      if (validLayerNames.contains(line)) layers.push(Layers.getLayerByLayerName(line));
    }
    return new SquadLayerFilter(layers, activeLayerFilter);
  }
  ...
  isMapHistoryCompliant(server, layer) {
    ...
    if (typeof layer === 'string') layer = Layers.getLayerByLayerName(layer);
  ...
  isGamemodeHistoryCompliant(server, layer) {
    ...
    for (let i = 0; i < Math.min(server.layerHistory.length, gamemodeHistoryTolerance); i++) {
      ...
      const historyLayer = Layers.getLayerByLayerName(server.layerHistory[i].layer);
  ...
  isFactionCompliant(server, layer) {
    ...
    const historyLayer = Layers.getLayerByLayerName(server.layerHistory[0].layer);
